	...
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			<view class="imageItem" v-bind:key="index" v-for="(path,index) in imageListData">
				<image :src="path" mode="aspectFill" :class="{'dragging':isDragging(index)}" draggable="true" @tap="previewImage" :data-index="index" @touchstart="start" @touchmove.stop.prevent="move" @touchend="stop"></image>
				<view v-if="isShowDel" :class="{'dragging':isDragging(index)}" class="imageDel" @tap="deleteImage" :data-index="index">x</view>
			</view>
	...
	export default {
		...
		mounted: function(){
			...
			if(this.showUploadProgress === false){
				this.showUploadProgress1 = false
			}else{
				this.showUploadProgress1 = true
			}
		},
		destroyed: function(){
